...
class MasteryController(object):
    ...
    def _isHangarRoute(routeInfo):
        ...
        return any(marker in normalized for marker in (
            '/hangar/{root}', 'hangar/{root}', '/garage/{root}', 'garage/{root}'))
    ...
    def enable(self):
        ...
        try:
            ...
            pass
        self._bindLobbyStateMachine()
        self._scheduleRouteFallback()

    def _bindLobbyStateMachine(self):
        lsm = getLobbyStateMachine()
        if lsm is self._lobbyStateMachine:
            return lsm
        self._unbindLobbyStateMachine()
        if lsm is not None:
            try:
                lsm.onVisibleRouteChanged += self._onVisibleRouteChanged
                self._lobbyStateMachine = lsm
            except Exception:
                logger.exception('failed to bind lobby route listener')
        return lsm

    def _unbindLobbyStateMachine(self):
        if self._lobbyStateMachine is not None:
            try:
    ...
    def _applyRouteFallback(self, generation):
        ...
        if generation != self._injectGeneration or not self._enabled:
            return
        self._lobbyReady = self._isLobbyAppReady()
        lsm = self._bindLobbyStateMachine()
    ...
    def _refresh(self):
        ...
        if xp is None or not self._isFresh(tankID, 'xp'):
            self._requestDistribution(tankID, 'xp', generation=self._vehicleGeneration)
        if moe is None or not self._isFresh(tankID, 'damage'):
            self._requestDistribution(tankID, 'damage', generation=self._vehicleGeneration)
    ...
    def _requestDistribution(self, tankID, distribution, attempt=1, generation=None):
        if generation is None:
            generation = self._vehicleGeneration
        ...
        try:
            BigWorld.fetchURL(
                ...
                lambda response, t=tankID, d=distribution, a=attempt, g=generation:
                    self._onApiResponse(t, d, response, a, g),
                None, _API_TIMEOUT, 'GET', None)
        except Exception:
            self._apiFailure(tankID, distribution, attempt, generation)
    ...
    def _apiFailure(self, tankID, distribution, attempt, generation):
        ...
        if attempt < _API_MAX_ATTEMPTS:
            ...
            BigWorld.callback(delay, lambda: self._requestDistribution(
                tankID, distribution, attempt + 1, generation))
        else:
            pending.discard(tankID)

    def _onApiResponse(self, tankID, distribution, response, attempt, generation):
        isXp = distribution == 'xp'
        pending = self._pendingXp if isXp else self._pendingMoe
        ...
        if isXp:
            ...
            if self._isCurrentVehicleResponse(tankID, generation):
